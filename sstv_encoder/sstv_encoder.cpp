c_sstv_encoder :: c_sstv_encoder
{
    m_Fs_kHz = 15;
    m_phase = 0
    m_sin_table = np.round(32767*np.sin(np.arange(1024)*2*np.pi/1024))
    m_frequencies = []
    m_tone = []
    m_residue_f16 = 0;
}

int16_t c_sstv_encoder :: output_sample(uint32_t frequency, uint16_t samples)
{
    uint32_t step = static_cast<uint64_t>(frequency)*(1<<32)/m_Fs;
...
int16_t c_sstv_encoder :: generate_tone(uint16_t frequency, uint32_t time_ms_f16)
{
...
void c_sstv_encoder :: generate_scottie(uint16_t width, uint16_t height)
{
  ...
  float colour_time_ms_f16 = ((width == 320)?138.240:88.064) * (1<<16);
  uint32_t pixel_time_ms_f16 = colour_time_ms_f16/width;
...
void c_sstv_encoder :: generate_martin(uint16_t width, uint16_t height)
{
  ...
  uint32_t hsync_pulse_ms = 4.862 * (1<<16);
  uint32_t colour_gap_ms = 0.572 * (1<<16);
  float colour_time_ms = ((width == 320)?146.342:73.216) * (1<<16);
  uint32_t pixel_time_ms_f16 = colour_time_ms_f16/width;
...
void c_sstv_encoder :: generate_sstv(e_sstv_tx_mode, uint16_t width, uint16_t height)
{
  ...
  if (mode == martin) generate_martin(tone_generator, width, height);
  else generate_scottie(tone_generator, width, height);
